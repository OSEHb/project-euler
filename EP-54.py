...
# к примеру, две восьмерки выигрывают две пятерки (см. пример 1 ниже). Если же достоинства карт у игроков одинаковы,
# к примеру, у обоих игроков пара дам, то сравнивают карту наивысшего достоинства (см. пример 4 ниже);
# если же и эти карты одинаковы, сравнивают следующие две и т.д.
# Файл poker.txt содержит одну тысячу различных ставок для игры двух игроков. В каждой строке файла приведены десять карт
# (отделенные одним пробелом): первые пять - карты 1-го игрока, оставшиеся пять - карты 2-го игрока. Можете считать,
# что все ставки верны (нет неверных символов или повторов карт), ставки каждого игрока не следуют в
...
high_card = 1  # Старшая карта: (Карта наибольшего достоинства).
one_pair = 2  # Одна пара: (Две карты одного достоинства).
two_pairs = 3  # Две пары: (Две различные пары карт).
three_kind = 4  # Тройка: (Три карты одного достоинства).
straight = 5  # Стрейт: (Все пять карт по порядку, любые масти).
flush = 6  # Флаш: (Все пять карт одной масти).
full_house = 7  # Фул-хаус: (Три карты одного достоинства и одна пара карт).
four_kind = 8  # Каре: (Четыре карты одного достоинства).
straight_flush = 9  # Стрейт-флаш: (Любые пять карт одной масти по порядку).
royal_flush = 10  # Роял-флаш: (Десятка, валет, дама, король и туз одной масти).
...
def name_hand(arg):
    ...
    if suit[0] == suit[1] == suit[2] == suit[3] == suit[4] and sum(nums) == 60:
        return royal_flush

    if suit[0] == suit[1] == suit[2] == suit[3] == suit[4] and nums[4] - nums[3] == 1:
        if nums[3] - nums[2] == 1 and nums[2] - nums[1] == 1:
            if nums[1] - nums[0] == 1:
                return [straight_flush, nums]

    if nums[0] == nums[1] == nums[2] == nums[3] or nums[1] == nums[2] == nums[3] == nums[4]:
        if nums[0] == nums[1] == nums[2] == nums[3]:
            return [four_kind, nums[0:4], nums[4]]
        else:
            return [four_kind, nums[1:], nums[0]]

    if nums[0] == nums[1] == nums[2] and nums[3] == nums[4] or nums[2] == nums[3] == nums[4] and nums[0] == nums[1]:
        if nums[0] == nums[1] == nums[2] and nums[3] == nums[4]:
            return [full_house, nums[0:3], nums[3:]]
        else:
            return [full_house, nums[2:], nums[0:2]]

    if suit[0] == suit[1] == suit[2] == suit[3] == suit[4]:
        return [flush, nums]

    if nums[4] - nums[3] == 1 and nums[3] - nums[2] == 1:
        if nums[2] - nums[1] == 1 and nums[1] - nums[0] == 1:
            return [straight, nums]

    if nums[0] == nums[1] == nums[2] or nums[1] == nums[2] == nums[3] or nums[2] == nums[3] == nums[4]:
        if nums[0] == nums[1] == nums[2]:
            return [three_kind, nums[0:3], nums[3:]]
        elif nums[1] == nums[2] == nums[3]:
            three = nums[1:4]
            del nums[1:4]
            others = nums
            return [three_kind, three, others]
        else:
            return [three_kind, nums[2:], nums[0:2]]

    if nums[0] == nums[1] and nums[3] == nums[4] or nums[0] == nums[1] and nums[2] == nums[3] or nums[1] == nums[2] and \
            nums[3] == nums[4]:
        if nums[0] == nums[1] and nums[3] == nums[4]:
            return [two_pairs, nums[3:], nums[0:2], nums[2]]
        elif nums[0] == nums[1] and nums[2] == nums[3]:
            return [two_pairs, nums[2:4], nums[0:2], nums[4]]
        else:
            return [two_pairs, nums[3:], nums[1:3], nums[0]]

    if nums[0] == nums[1] or nums[1] == nums[2] or nums[2] == nums[3] or nums[3] == nums[4]:
        if nums[0] == nums[1]:
            return [one_pair, nums[0:2], nums[2:]]
        elif nums[1] == nums[2]:
            pair = nums[1:3]
            del nums[1:3]
            others = nums
            return [one_pair, pair, others]
        elif nums[2] == nums[3]:
            pair = nums[2:4]
            del nums[2:4]
            others = nums
            ...
            return [one_pair, nums[3:], nums[0:3]]

    else:
        return [high_card, nums]


for hand in hands:
    player_1 = name_hand(hand[0])
    player_2 = name_hand(hand[1])
    ...
    if player_1[0] > player_2[0]:
        ...
        if player_1[0] == 9 and player_2[0] == 9:
            if player_1[1][4] > player_2[1][4]:
                win_PL_1 += 1
            elif player_1[1][4] < player_2[1][4]:
                win_PL_2 += 1

        if player_1[0] == 8 and player_2[0] == 8:
        ...
        if player_1[0] == 7 and player_2[0] == 7:
            if player_1[1][0] > player_2[1][0]:
                win_PL_1 += 1
            elif player_1[1][0] < player_2[1][0]:
                win_PL_2 += 1
        ...
        if player_1[0] == 6 and player_2[0] == 6:
            if player_1[1][4] > player_2[1][4]:
                win_PL_1 += 1
            elif player_1[1][4] < player_2[1][4]:
                win_PL_2 += 1
            else:
                if player_1[1][3] > player_2[1][3]:
                    win_PL_1 += 1
                elif player_1[1][3] < player_2[1][3]:
                    win_PL_2 += 1
                else:
                    if player_1[1][2] > player_2[1][2]:
                        win_PL_1 += 1
                    elif player_1[1][2] < player_2[1][2]:
                        win_PL_2 += 1
                    else:
                        if player_1[1][1] > player_2[1][1]:
                            win_PL_1 += 1
                        elif player_1[1][1] < player_2[1][1]:
                            win_PL_2 += 1
                        else:
                            if player_1[1][0] > player_2[1][0]:
                                win_PL_1 += 1
        ...
        if player_1[0] == 5 and player_2[0] == 5:
            if player_1[1][4] > player_2[1][4]:
                win_PL_1 += 1
            elif player_1[1][4] < player_2[1][4]:
                win_PL_2 += 1

        if player_1[0] == 4 and player_2[0] == 4:
            if player_1[1][0] > player_2[1][0]:
                win_PL_1 += 1
            elif player_1[1][0] < player_2[1][0]:
                win_PL_2 += 1
                ...
                if player_1[2][1] > player_2[2][1]:
                    win_PL_1 += 1
                elif player_1[2][1] < player_2[2][1]:
                    win_PL_2 += 1
                else:
                    if player_1[2][0] > player_2[2][0]:
                        win_PL_1 += 1
        ...
        if player_1[0] == 3 and player_2[0] == 3:
            if player_1[1][0] > player_2[1][0]:
                win_PL_1 += 1
            elif player_1[1][0] < player_2[1][0]:
                win_PL_2 += 1
                ...
                if player_1[2][0] > player_2[2][0]:
                    ...
                    if player_1[3][0] > player_2[3][0]:
                        win_PL_1 += 1
                    elif player_1[3][0] < player_2[3][0]:
                        win_PL_2 += 1

        if player_1[0] == 2 and player_2[0] == 2:
            if player_1[1][0] > player_2[1][0]:
                win_PL_1 += 1
            elif player_1[1][0] < player_2[1][0]:
                win_PL_2 += 1
            else:
                if player_1[2][2] > player_2[2][2]:
                    win_PL_1 += 1
                elif player_1[2][2] < player_2[2][2]:
                    win_PL_2 += 1
                else:
                    if player_1[2][1] > player_2[2][1]:
                        win_PL_1 += 1
        ...
        if player_1[0] == 1 and player_2[0] == 1:
            if player_1[1][4] > player_2[1][4]:
                win_PL_1 += 1
            elif player_1[1][4] < player_2[1][4]:
                win_PL_2 += 1
...
print('PL-1:', win_PL_1, 'PL-2:', win_PL_2)
